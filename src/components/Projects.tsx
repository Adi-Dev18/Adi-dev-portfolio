...
export function ProjectCard({ project, index, onClick }: ProjectCardProps) {
  return (
    <motion.div
      ...
      className="group flex flex-col overflow-hidden rounded-2xl bg-[#0d0f1a] border border-white/[0.08] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.4)] hover:border-white/[0.12] cursor-pointer"
      onClick={onClick}
    >
      ...
      <div className="flex flex-1 flex-col px-6 pt-2 pb-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {project.tech.slice(0, 4).map((tech) => (
            <span
              key={tech}
              className="inline-block rounded-full border border-white/[0.12] bg-white/[0.04] px-2.5 py-1 text-[11px] font-medium text-white/70 transition-colors duration-200 group-hover:border-white/[0.18] group-hover:bg-white/[0.06] group-hover:text-white/90"
            >
              {tech}
            </span>
          ))}
